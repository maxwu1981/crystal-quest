...
export class StatusScene {
  ...
  render(ctx) {
    ...
    drawText(ctx, `HP ${m.hp}/${s.maxHp}   响 ${m.mp}/${s.maxMp}`, 76, 16 + LINE_H * 2);
    tags.forEach((t, k) => drawText(ctx, t.name, 200 + k * 28, 16 + LINE_H, { color: t.color }));
    drawText(ctx, `回声 ${m.exp}   再听 ${Math.max(0, expForLevel(m.level + 1) - m.exp)} 点`, 76, 16 + LINE_H * 3, { color: '#8a8468' });
    ...
    drawText(ctx, '言灵', 24, 154, { color: '#8a8468' });
