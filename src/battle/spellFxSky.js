...
// 雷的「连闪几下」和光的「一直亮着」是全套七种里最容易做出癫痫风险的两个写法，所以
// 两边都只闪一次——雷是**亮起一次然后拖尾**，光是**慢慢涨到峰值再慢慢退**。另外两条
// 来自火焰那次三版试错（fxKit.js 的 tongue() 上面）：**画形状不撒粒子**；
// **按目标大小 o.w/o.h 缩放**，同一发魔法罩在小怪和 boss 身上不能一样大。
...
// step 给大一点（光柱一百多逻辑像素高）能省掉几千次 fillRect，纵向渐变分档到 2–3
// 物理像素看不出来；横向宽度仍按 PX 走，锥度该平滑还是平滑。
...
// 干净得像线框（第一版实测：三条一起走活像三个呼啦圈）。这里拆成 44 段短弧，
// 每段亮度按角度起伏（相位构造时定死），环上就有明暗参差，不再是一条闭合的硬线。
...
function halo(ctx, x, y, r, color, a, thick, ph, flat = 0.42) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    // 相邻两段**严丝合缝**（+1，不是 +0.9 也不是 +1.25）：留缝的那版是一圈虚线，
    // 叠着画的那版在重叠处透明度翻倍成了一串珠子。参差只能来自亮度，不能来自缺口
...
export const SKY_FX = {
  // 雷：兆(0–.30) → 劈(.30–.45) → 爆(.40–.80) → 余(.62–1)　（段落故意首尾交叠，不留空档）
  //   兆 **局部**：天压暗，云底的脉络先亮，目标脚下窜起静电细丝——「这里要挨劈了」
  //   劈 **全屏**：一道带二级分叉的枝干从画面顶端劈到脚下；全场只闪这一次
  //   爆 **地面**：落点向两侧爬开的地电 + 两圈冲击环 + 一道离子残痕
  //   余 **贴身**：电弧在目标身上零星跳几下，暗色退干净。尺度一路收：全屏→地面→贴身
  thunder: (x, y, rng, o = {}) => {
    ...
    const veins = Array.from({ length: 3 }, (_, i) => ({
      ...
      pts: branch(rng, x + rng.int(-78, -22), 4 + i * 5, x + rng.int(22, 78), 5 + i * 5 + rng.int(-3, 3),
        1.3, 0.4, 2.6, 8) }));
    // 地面爬电：落点向两侧沿地面爬开的枝干。**这是形状不是散点**——旧版那圈四散的
    // 小方块像噪声，爬电才读得出「电顺着地面走掉了」。六条的落脚 y 必须错开（±7）：
    // 第一版全压在 foot 这一条线上，六条叠起来直接连成一根横杠，正是禁止的那种硬边
    ...
    // 电荷：从画面外收进目标脚下。**终点必须各自错开**——十四颗收敛到同一个坐标时，
    // 每颗只有 0.08 的透明度，叠十四层也是 0.69：劈之前那两帧脚边杵着一颗白方块
    ...
    return { t: 0, dur: 1.25, render(ctx, p) {
      ...
      // ---- 爆 ----
      // 离子通道：主干退干净之后还留一道极淡的白痕——**这就是「亮一次然后拖尾」
      // 里的拖尾，用它代替连闪几下**。画宽而淡：细而实的那版像一根没擦干净的线
      ...
      // ---- 余 ----
      // 落点还烫着：一摊压得很低的余光陪到最后一帧，不留的话尾段空得像特效提前结束。
      // envelope 必须**从劈完就开始涨**（0.42→0.60），不能挂在 af>0 里面——
      // 挂在里面那版一到 p=0.62 就凭空出现 0.30 的加光，体检量到帧间跳变 0.293
      const af = seg(p, 0.62, 1);
      glow(ctx, x, foot - 2, 42 * kx, 'rgb(90,130,235)', seg(p, 0.42, 0.60) * (1 - af) ** 1.4 * 0.30);
      if (af > 0) {
        ctx.save();
  ...
  // 光：兆(0–.28) → 降(.22–.52) → 绽(.44–.80) → 升(.70–1)　（段落故意首尾交叠）
  //   兆 **局部**：脚下先浮出一个环，环上冒起细光丝；头顶的天开始亮——光是先「照过来」的
  //   降 **全屏**：一束（九条）光柱从画面顶端漫下来罩住目标，落地时全场只闪这一次
  //   绽 **中景**：六道光束从目标身上射出 + 三重光环摊开。六道是照着六水晶／六堆来的
  //   升 **局部**：柱收回天上，脚下的光丝一束束升起穿过目标散掉。尺度：局部→全屏→局部
  // 光在剧情里是六水晶合成的那一样东西（docs/主线设计.md「見得光」），
  // 所以它不能只是「白色的火」——得有**仪式感**：慢起、成束、对称、有环。
  light: (x, y, rng, o = {}) => {
    const kx = S(o.w || 44), ky = S(o.h || 44);
    ...
    const beams = Array.from({ length: N }, (_, i) => {
      ...
      return { dx: (i - (N - 1) / 2) * 4.6, dy: rng.int(-5, 4),
        ...
        // 每条都很透（中间那条也只有 0.31）：九条叠起来就已接近不透明，
        // 第一版 0.44 时整只怪被柱子吃掉，玩家看不见自己在打谁
        a: 0.11 + c * 0.20, d: rng.next() * 0.26, ph: rng.next() * 6.28 };
    });
    // 六道放射：主束六条（每 60°），中间插六条短的填空隙——仍是六重对称，只是不稀疏。
    // **起点空出中心 14%**：十二道三层全从同一点长出来时圆心叠成一块死白，
    // 目标消失在星星中间；空出一小圈之后，光是「从怪身上四周射出来」的
    const rays = Array.from({ length: 12 }, (_, i) => {
      ...
      return { long, pts: Array.from({ length: 7 }, (_, j) => {
        const t = j / 6, r = len * (0.14 + t * 0.86), aa = a0 + Math.sin(t * 2.4) * 0.10;
