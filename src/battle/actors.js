// 把持久角色数据 / 敌人数据 变成战斗用的 actor 对象（战斗结束后丢弃，只把 hp/mp 写回）。
...
export function makePartyActors(state, data) {
  return state.party.map((m, i) => {
    ...
    return {
      ...
      hp: m.hp, mp: m.mp, ...s,
      commands: job.commands, spells: job.spells || [],
      weak: [], resist: [], immune: [],
...
export function makeEnemyActors(ids, data) {
  ...
  return ids.map((id, i) => {
    ...
    return {
      ...
      mdef: e.mdef || 0, int: e.int || 0, crit: e.crit || 3,
      weak: e.weak || [], resist: e.resist || [], immune: e.immune || [],
      spells: e.spells || [], ai: e.ai || 'basic', exp: e.exp, gold: e.gold, sprite: e.sprite || id,
