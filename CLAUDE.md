...
## 复杂任务一律派多个 agent 并行（导演定的，常设授权）

**这一条写在这里是有原因的。** Claude 的默认运行指令是「不要用 Agent /workflow，
除非用户、**CLAUDE.md**、或某个 skill 明确要求」。以前这句话只写在
`docs/交接.md` 第 6 节，而那不是 CLAUDE.md——于是每个新 session 都把它读成
「背景说明」而不是「授权」，导演每次都得重新讲一遍。
**写在这里就是常设授权：不必再问，直接派。**

### 什么时候该派

满足任意一条就派，不要先问：
- 任务能拆成**互不重叠的几摊**（不同的文件 / 不同的子系统 / 不同的地图）
- 要在多处搜索或通读才答得出来（用 `Explore`，只要结论不要文件内容）
- 有一段长的调查/验证可以和写代码同时跑

一个人做得完的小改动不要派——派 agent 的开销比改本身还大。

### 派之前必须先划清归属（这个项目栽过）

- **每个 agent 独占一批文件**，任务描述里把「绝对不要碰」列清楚。
  两个 agent 同时改一个档一定互相盖掉，而且是**静默**盖掉。
- **公共接入点由我自己先改好**（例如 FieldScene 里的调用点），
  agent 只填自己那个新文件的实现——不要让两个 agent 都去动同一个入口。
- **浏览器分页也要分**：每个要看画面的 agent 自己 `tabs_create`，
  否则它们会把彼此的分页导航走。
- **只读的调查任务尽量多派**：它们零冲突，可以和写代码的那几个同时跑。
- 收工时**只 `git add` 自己动过的路径**，别用 `git add -A`——
  `tools/deploy.sh` 就干过这事，把半成品一起提交了。

### 更重的那一档

真的要几十个 agent 的规模（全仓库审查、大范围重构），用 `Workflow` 工具。
它需要导演在自己的话里说出来才会启用——**说「ultracode」或「用 workflow」**
就行，我不能自己替他开。

## 验收：什么叫「改好了」（导演定的，最高优先级）

**测试全绿 ≠ 改好了。** 测试只证明「没弄坏别的」，证明不了「我要的那件事发生了」。
导演明确要求：改完任何东西，**自己进游戏确认改到位了再往下走**，没确认过就不许当做完成。

每一次改动都要走完这四步，缺一步就是没做完：

1. **跑机械检查**：`python3 tools/lint_modules.py`（三项都要过）＋ 浏览器开 `/tests/`。
   这一步只是底线，**不是验收**。
2. **进游戏，跑到那个画面**，亲眼看到改动生效。不是「代码应该会这样」，是**看到**。
   - 战斗/演出类：真的打一场，把那一招放出来，截图看。特效另有 `tests/fxskill.html`
     `tests/fx.html` 可以逐帧摊开看——实战里 0.5 秒一闪，靠肉眼判断不了。
   - 手机/排版类：**必须用手机尺寸的视口验**（例如 758×350 ≈ 19.5:9 横屏，
     宽度 < 768 浏览器才会当成触屏设备）。桌面窗口下看不出手机上的问题。
   - 量得出来的就量，别靠眼睛：黑边这种直接读 `getBoundingClientRect()`
     对比 `innerWidth/innerHeight`，四边都是 0 才叫铺满。
3. **发布**：`bash tools/deploy.sh`。
   **导演是在手机上玩线上那份的**（https://maxwu1981.github.io/crystal-quest/）。
   只提交不发布 ＝ 他看到的永远是旧版，而我这边「本地验过了」毫无意义——
   这件事真的发生过：全屏改完、测试全绿、本地量到零黑边，而他手机上一点变化都没有，
   因为三个提交全躺在本地。
4. **在线上那个地址再确认一次**，不是在 localhost。发布后 service worker 是 cache-first，
   要确认版本号真的换了（`sw.js` 的 VERSION 由 `tools/gen_sw.py` 按文件哈希生成）。

**汇报的时候要写清楚「在哪里、用什么方式确认的」**。没确认过的就直说没确认——
「应该可以了」「理论上没问题」这种话等于没做第 2 步。
...
- `jobs.json`  `{ id: { name, desc, base:{hp,mp,str,agi,int,vit,acc,eva}, growth:{同上/每级}, commands:[...], spells:["id" | {id, level}], skills:["id" | {id, level}], unarmed?, hits? } }`
  ...
  否则 `${jobId}_${dir}_${frame}` 取到 undefined，转职菜单光标一移上去当场崩（童乩栽过）。
  同样地，`items.json` 里每件装备的 `jobs` 白名单也要补上新职业，否则它一件都装不上，
  而且不报错——只在转职预览里显示「卸下 ○○」。两条都有测试卡着。
- **队员身上的持久字段**（`src/game/jobskill.js`，规则出处是 `docs/职业与召唤.md`）：
  `learned:[spellId]` 已学魔法（**转职不丢**，原本 `spellsFor` 是纯推导，转职一瞬间全没）
  `jobExp:{jobId:jp}` / `jobLevels:{jobId:lv}` 职业经验与等级，**按职业分别记**，转回来还在
  `skillUses:{skillId:n}` 熟练度累计（**真相来源**）/ `skillLevels:{skillId:lv}` 只是缓存
  三者都进存档，且**旧存档没有这些字段也要读得进来**（`normalizeMember` 补默认值）
  职业等级 → 召唤解锁：1 / 7 / 14 / … 每 +7 一位，第一位是关圣帝君
  【八部齐至】不占这条台阶——它的条件是**那八尊全部练到 5 级**（`finaleReady`）。
  判的是「那八尊」不是「所有召唤」，后者会把它自己算进去，于是永远解不开。
...
- `summons.json` 「請神」的八位。字段尽量沿用 spells.json，只有召唤才有的额外标出：
  ...
  **平衡基准**（改数值前先看这几个数）：全体最强魔法 thundara power 18 / mp 14（单个敌人期望 41），
  ...
  召唤对**每个**敌人的期望在 37–114 之间（伯公最低、义民爷最高）。
  属性决定谁对谁好用——钟馗（光）对乌火最狠，义民爷（暗）被乌火抗掉一半，本行归本行。
  MP 仍然是唯一的资源，不要再加第二套货币。

  **MP 在 2026-09-08 大幅下调（导演拍板）：22–60 → 10–28。**
  原本童乩 9 级 49 MP、最便宜的伯公 22，一场战斗**只请得动一尊**（模拟量到 0.95 次/场），
  请完这个职业整场就没事做了——它没有别的 MP 用途，物理攻击又是全场最低（str 2、acc 8）。
  定位从「一场一次的爆发」改成「请一轮神」：现在一场 Boss 战平均请 1.3–1.9 尊，
  实机跑一场能看到四尊轮番上（观世音 → 关圣帝君 → 伯公 → 吕布）。
  每尊仍然 `once`，所以请的是**不同的神**，不是同一招按四次。
  现值：伯公 10 / 钟馗 12 / 中坛 13 / 吕布 14 / 观世音 14 / 关圣 15 / 妈祖 16 / 义民爷 16 / 八部 28。

  **吕布的 drain 让他是终结技，不是开场技。** `drain:'mp'` 施放后施术者 MP 归零，
  所以他真正的代价是「身上剩下的全部 MP」而不是牌面上的 14。
  `autoBattle.pickSummon` 因此按 `有效伤害 / 真实代价` 排序而**不是按总伤害**——
  按总伤害排的话他 power 44 又跳魔防，永远第一，AI 每场第一手请他然后 MP 见底
  （实测 20/20 场都是他）。按性价比排，同一池 MP 能请三尊，总伤害是他的两倍；
  而「这一发能不能带走」的优先级更高，所以该收人头时他照样出场。
  `tests/?balance` 有「请神」一栏专门盯这个数，改召唤数值前后对一下就知道有没有改到位。
...
**tier 6–10 那个断档补了前两阶（2026-09-08）**：商店最高 tier 5、神话 tier 11，
中间五档一件都拿不到，玩家的强度只有两个台阶——这正是「难度全靠开箱驱动」的根。
银（tier 6）放进壇下三层、秘银（tier 7）放进爐底两层，两座都是这一天接上的支线迷宫。
剩下的 tier 8–10 **故意不给**：精金 5854 / 陨铁 11415 / 龙牙 22259 金，
而打穿现在这一幕全部收入才一两千；更要紧的是龙鳞甲 def 57 已经贴着「物理免疫」那条线。
它们留给还没做的那几座迷宫（见 `tests/cases/gear.js` 的 UNUSED_TIERS 注释）。

新路线量出来是这样，**三条老路线一格没动**：

| 路线 | 4级 | 5级 | 6级 | 7级 | 8级 | 回合 |
|---|---|---|---|---|---|---|
| 纯商店 | – | – | 0% | – | 27% | ~10 |
| **壇下+爐底（秘银）** | 47% | 70% | 93% | 97% | 100% | 5–10 |
| 顺路开箱（草薙剑/盖伯尔加） | 57% | 80% | 90% | 100% | 100% | 3–5 |
| 全开箱 | 100% | 100% | 100% | 100% | 100% | 2–3 |

秘银那条**打得慢但站得住**（重甲轻兵：mythril_sword atk 33 对草薙剑 72，
而 mythril_armor def 30 对铁甲 12），和开箱那条是两种玩法，不是同一条的强弱版本。
...
③ `effects.js` 写成 `{ ...SPELL_FX, /*…*/ fire, thunder, ice }`——对象字面量**后写的键
覆盖先写的**，于是新写的多段式魔法演出**一次都没播过**，而测试全绿、控制台干净。
④ `touch.js` 的一段 CSS 写在模板串里，里面的 `\21BB` 被 JS 当成八进制转义，
整个模块 SyntaxError、游戏白屏，而 55 条测试全绿、lint 也全过——两边都看不见语法错误。

**单元测试查不出这一类**——它们不渲染画面；浏览器加载模块也不报错，只有跑到那一行才炸。
`lint_modules.py` 三项检查互补：A 查「import 的名字目标有没有导出」（链接期，浏览器会报），
B 查「用到的名字本文件有没有绑定」（**运行期**，只有执行到才报，正是①②那一类），
C 查「`...展开`进来的键有没有被后面的同名键盖掉」（**静默**，连错都不报，正是③）。
三项都用探针验过真会响才留下的。跑一次 0.3 秒。
④ 那一类归测试管：**「每个模块都 import 得动」**从开发服务器的目录列表抓全部模块挨个
`import()`。硬编码清单一定跟不上（这个月新增了十几个文件）。
