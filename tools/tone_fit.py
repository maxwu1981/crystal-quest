...
**默认阈值是起点，不是判据。** 用默认值扫一遍全部怪物会报出几只「出界」，
但那多半不是漂移：山猪本来就是全组最饱和的一只（旧版 0.729），
它的目标就该是 0.73 而不是默认的 0.68；另有几只是当初正好压到阈值上、
派生时留下的千分之几舍入残差。**看到报告先去比它自己的旧版**，
别拿默认阈值当对错的标准——否则会把一只本该鲜艳的怪越修越灰。
